...
def sd1_inject_attention_wrappers(module, callback_func=None, path='', collect_wrappers=None) -> dict:
    """
    ...
    if isinstance(module, Attention):
        ...
        for k, v in list(module.named_children()):
            sd1_inject_attention_wrappers(v, callback_func, path + '.' + k, collect_wrappers)
    return collect_wrappers


def sd1_perform_single_image_diffusion_step(pipe, img: np.ndarray, timestep, device, torch_dtype):
    """
...
class StableDiffusion1AttentionAggregator(object):
    ...
    def extract_attention(self, image: np.ndarray) -> torch.Tensor:
        ...
        """
        self.current_merged_tensor = None
        sd1_perform_single_image_diffusion_step(
            pipe=self.pipe,
...
def main():
    ...
    aggr = StableDiffusion1AttentionAggregator(device='cuda:1', attention_resolution=110)
    print(aggr.path_to_weight_dict.keys())
